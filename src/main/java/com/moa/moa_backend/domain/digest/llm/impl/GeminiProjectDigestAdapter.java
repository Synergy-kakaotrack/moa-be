...
public class GeminiProjectDigestAdapter implements ProjectDigestGeneratorPort {
    ...
    private String buildPrompt(
        ...
        if (kind == DigestKind.CUSTOM) {
            ...
            ### 최우선 규칙(강제)
            - 사용자 요구사항에 "순서/구성/섹션 제목/형식"이 명시되어 있으면 그 구조를 반드시 따른다.
            - 이 경우 아래의 '기본 출력 형식'템플릿을 절대 먼저 출력하지 말고, 사용자 구조 외의 섹션을 임의로 추가하지 마라
            - 사용자 요구사항이 구조를 전혀 제공하지 않을 때만 '기본 출력 형식'을 사용하라. 
            
            ### 기본 출력 형식(조건부)
            - 사용자가 형식/순서를 지정하지 않은 경우에만 아래 형식을 사용하라:
            
                ## 한줄 요약
                > (1~2줄로 프로젝트의 내용 및 목적 요약)
            ...
            ### 길이 제한(강제)
            - 전체 분량은 900~1400자(공백 포함)로 제한하라.
            - 항목 수를 채우기 위해 억지로 늘리지 마라.
            ...
            - 각 항목 형식:
                - **(무엇을 하기로 했는지 - 결정 문장만, "결정:" 같은 접두어 금지)**
                - 근거: (왜 그렇게 했나)
